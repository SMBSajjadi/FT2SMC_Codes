...
alpha1=5; %% ba kahesh alpha sorate hamgeraee afzayesh miabad ama maghadire Dhat niz ziad mishavad va voroodiha niz kami navasanitar mishavand dar vaghe alpha be noee zamane hamgeraeeye Z ra moshakhas mikonad
...
Eta=2.3; %% kaheshe eta baese kaheshe shadide mizan D hat ha mishavad ama az tarafi zamane hamgeraee ra be shedat afzayesh midahad
% dar eta barabare 3 adade alpha taghriban haman zamane residane z be
% meghdare hadaf ast
...
%     Dhat_sai(i)= (1/I_z)*((u_z(i)/(4*kt))+(u_sai(i)/(4*kd))-(u_teta(i)/(2*kt*l)))*(f1*kt*l+f3*kd) ;
    ...
    S2_z(i+1)=X12(i)-X11_d_moshtagh(i)+alpha1*(X11(i)-X11_d(i))+beta1z*((X11(i)-X11_d(i))^(p1/q1))+S_z(i+1);
    S2_d_z(i+1)=(S2_z(i+1)-S2_z(i));
    vr_eq_z(i+1)=g+X11_d_moshtagh2m(i)-(alpha1*(X12(i)-X11_d_moshtagh(i)))-(beta1z*(p1/q1)*((X11(i)-X11_d(i))^((p1-q1)/q1))*(X12(i)-X11_d_moshtagh(i)))-Dhat_z(i+1);
    ...
    S2_sai(i+1)=X6(i)-X5_d_moshtagh(i)+alpha1*(X5(i)-X5_d(i))+beta1say*((X5(i)-X5_d(i))^(p1/q1))+S_sai(i+1);
    S2_d_sai(i+1)=(S2_sai(i+1)-S2_sai(i));
    vr_eq_sai(i+1)= X5_d_moshtagh2m(i)-(alpha1*(X6(i)-X5_d_moshtagh(i)))-(beta1say*(p1/q1)*((X5(i)-X5_d(i))^((p1-q1)/q1))*(X6(i)-X5_d_moshtagh(i)))-Dhat_sai(i+1);
...
% 
% subplot(2,2,1)
% plot(T(1:N-1), X7(1:N-1),'b','linewidth',1)
% hold on
% grid on
% plot(T(1:N-1), X7_d(1:N-1),'--.r','linewidth',1.2)
% xlabel('Time (s)','FontWeight','Bold')
% legend('x','x_{d}')
% ylabel('x (m)')
% 
% subplot(2,2,2)
% plot(T(1:N-1), X9(1:N-1),'b','linewidth',1)
% hold on
% grid on
% plot(T(1:N-1), X9_d(1:N-1),'--.r','linewidth',1.2)
% xlabel('Time (s)','FontWeight','Bold')
% legend('y','y_{d}')
% ylabel('y (m)')
...
% subplot(2,2,3)
% plot(T(1:N-1), X11(1:N-1),'b','linewidth',1)
% hold on
% grid on
% plot(T(1:N-1), X11_d(1:N-1),'--.r','linewidth',1.2)
% xlabel('Time (s)','FontWeight','Bold')
% legend('z','z_{d}')
% ylabel('z (m)')
% 
% subplot(2,2,4)
% plot(T(1:N-1), X5(1:N-1),'b','linewidth',1)
% hold on
% grid on
% plot(T(1:N-1), X5_d(1:N-1),'--.r','linewidth',1.2)
% xlabel('Time (s)','FontWeight','Bold')
% legend('\psi','\psi_{d}')
% ylabel('psi (rad)')
...
NORM = [norm(real(e_D_x(end-100:end)))
               norm(real(e_D_y(end-100:end)))
               norm(real(e_D_z(end-100:end)))
               norm(real(e_D_phi(end-100:end)))
               norm(real(e_D_theta(end-100:end)))
               norm(real(e_D_psi(end-100:end)))];
